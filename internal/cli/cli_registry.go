...
func newCommandRegistry() []*command {
	return []*command{
		...
		},
		{
			...
			Summary: "Show tasks",
			Flags: []flagSpec{
				flagSpec{"--tag", flagValue, "Filter by tag"},
				actorFlag,
				jsonFlag,
				flagSpec{"--all", flagBoolean, "Include completed tasks"},
			},
			Run: cmdStatus,
		},
		{
		...
		},
		{
			...
			Flags: []flagSpec{
				flagSpec{"--capabilities", flagValue, "Filter by comma-separated capabilities"},
				jsonFlag,
				flagSpec{"--ready", flagBoolean, "Show only ready tasks"},
			},
			Run: cmdNext,
		},
		{
		...
		},
		{
			...
			Flags: []flagSpec{
				actorFlag,
				flagSpec{"--capabilities", flagValue, "Capabilities to match ready work against"},
				flagSpec{"--objective", flagValue, "Task ID to treat as the objective"},
				flagSpec{"--limit", flagValue, "Maximum tasks per section"},
				flagSpec{"--events", flagValue, "Maximum recent events"},
				jsonFlag,
			},
			Run: cmdBootstrap,
		},
		{
			...
			Flags: []flagSpec{
				actorFlag,
				flagSpec{"--caps", flagValue, "Comma-separated capabilities to register"},
				flagSpec{"--desc", flagValue, "Human-readable description"},
				flagSpec{"--max-load", flagValue, "Maximum concurrent leases"},
				jsonFlag,
			},
			Run: cmdAgentCard,
		},
		{
			...
			Flags: []flagSpec{
				actorFlag,
				jsonFlag,
				flagSpec{"--all", flagBoolean, "Include capabilities with no demand"},
			},
			Run: cmdCaps,
		},
		{
		...
		},
		{
			...
			Flags: []flagSpec{
				flagSpec{"--done", flagBoolean, "Simulate completion"},
				flagSpec{"--claim", flagBoolean, "Simulate claiming"},
				flagSpec{"--block", flagBoolean, "Simulate blocking"},
				jsonFlag,
			},
			Run: cmdWhatIf,
		},
		{
			...
			Flags: []flagSpec{
				flagSpec{"--dot", flagBoolean, "Emit Graphviz DOT"},
				jsonFlag,
			},
			Run: cmdGraph,
		},
		...
		// Reactivity.
		{
			...
			Flags:   []flagSpec{flagSpec{"--poll", flagValue, "Refresh interval"}},
			Run:     cmdWatch,
		},
		{
			...
			Flags: []flagSpec{
				flagSpec{"--limit", flagValue, "Maximum events per page"},
				jsonFlag,
			},
			Run: cmdEvents,
		},
		...
		// Integrations.
		{
			...
			AllowsUninitializedProject: true,
			Flags:                      []flagSpec{flagSpec{"--stdio", flagBoolean, "Serve over stdio"}},
			Run:                        cmdMCP,
		},
		{
		...
		},
		{
			...
			AllowsUninitializedProject: true,
			Flags: []flagSpec{
				flagSpec{"--command", flagValue, "Path to the todo binary to register"},
				flagSpec{"--force", flagBoolean, "Overwrite existing configuration"},
				flagSpec{"--check", flagBoolean, "Report integration status without writing"},
				flagSpec{"--live", flagBoolean, "Perform a live MCP handshake check"},
			},
			Run: cmdIntegrate,
		},
		{
			...
			AllowsUninitializedProject: true,
			Flags: []flagSpec{
				flagSpec{"--host", flagValue, "Host to evaluate"},
				flagSpec{"--timeout", flagValue, "Per-scenario timeout"},
				flagSpec{"--model", flagValue, "Explicit model ID"},
				flagSpec{"--suite", flagValue, "Suite version"},
				flagSpec{"--run", flagBoolean, "Execute the suite; may consume paid usage"},
				jsonFlag,
				flagSpec{"--include-events", flagBoolean, "Include raw host events"},
				flagSpec{"--keep-workspace", flagBoolean, "Retain disposable workspaces"},
			},
			Run: cmdConformance,
		},
		...
		},
		{
			...
			Flags:   []flagSpec{flagSpec{"--markdown", flagBoolean, "Emit Markdown"}},
			Run:     cmdExport,
		},
		{
		...
		},
		{
			...
			Flags:   []flagSpec{flagSpec{"--output", flagValue, "Destination path"}},
			Run:     cmdBackup,
		},
		{
		...
		},
		{
			...
			Flags: []flagSpec{
				flagSpec{"--keep-events", flagValue, "Events to retain"},
				flagSpec{"--receipts-before", flagValue, "Drop receipts older than this"},
				flagSpec{"--dry-run", flagBoolean, "Report without writing"},
				jsonFlag,
			},
			Run: cmdCompact,
		},
		{
			...
			Flags:   []flagSpec{flagSpec{"--fix", flagBoolean, "Repair what can be repaired"}},
			Run:     cmdDoctor,
		},
		{
