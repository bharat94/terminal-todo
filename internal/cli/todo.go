...
var (
	...
	// Version is stamped by the release build through -ldflags. When it is
	// unset — which is the case for `go install`, since ldflags cannot be
	// passed through it — resolveVersion falls back to the module version the
	// toolchain recorded in the binary.
	Version = "dev"
)

// version is the resolved version string used in output.
...
// Execute runs the CLI. It is the single entry point shared by the module
// root binary and cmd/todo.
func Execute() {
	if len(os.Args) < 2 {
		printUsage()
		os.Exit(1)
